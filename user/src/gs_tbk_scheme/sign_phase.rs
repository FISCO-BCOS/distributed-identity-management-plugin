...
impl User
{
	...
	//info!("third point!");
        ...
        let script_path = "/home/test/fisco/personal-info/personal-info/dist/info_run.sh";
        //info!("2-222");
        let script_args = ["register",&self.name,user_path.to_str().unwrap()];

        // save original work dir
        let original_dir = env::current_dir().expect("Failed to get current directory");
        let java_project_dir = PathBuf::from("/home/test/fisco/personal-info/personal-info/dist");
        env::set_current_dir(&java_project_dir)
            .expect("Failed to change current directory");
        //info!("2-333");
