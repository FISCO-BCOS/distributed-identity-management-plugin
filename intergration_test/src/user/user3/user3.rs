...
{
    // 初始化 日志记录器
    let log_path = String::from(env::current_dir().unwrap().as_path().to_str().unwrap())+"/src/user/user3/config/config_file/log4rs.yaml";
    log4rs::init_file(log_path, Default::default()).unwrap();
    
    let config_path  = String::from(std::env::current_dir().unwrap().as_path().to_str().unwrap())+"/src/user/user3/config/config_file/user_config.json";
                        ...
                        gs_tbk_scheme::messages::proxy::common_msg::GSTBKMsg::JoinIssueMsg(join_msg) => 
                        {
                            match join_msg 
                            {
                                ...
                                gs_tbk_scheme::messages::proxy::common_msg::JoinIssueMsg::ProxyToUserJoinIssuePhaseThreeP2PMsg(msg) => 
                                {
                                    ...
                                    let path  = "src/user/user3/info/join.json";
                                    user_path.push(path);
